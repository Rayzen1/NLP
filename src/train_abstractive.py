...
def train_abs_single(args, device_id):
    ...
    torch.backends.cudnn.deterministic = True
    def train_iter_fct():
        test = data_loader.Dataloader(args, load_dataset(args, 'train', shuffle=True), args.batch_size, device,
    ...
    train_iter_fct()
    model = AbsSummarizer(args, device, checkpoint, bert_from_extractive, topic_embeddings=te )
    if (args.sep_optim):
